...
| Editable seven‑day plan | mostly done | The runway now reads the person's saved sleep plans (new `WakeSource.plan`), so it shows the same times as tonight's episode, the reminders and the alarm. The runway had ignored them. Each day's detail names its main constraint (`ScheduleFriction`) and can **set** that night's bed and wake: stored as a one‑night plan (`PersonalSetup.setNightPlan`/`clearNightPlan`), so it **locks** the night for every surface, can be changed, or cleared back to the usual. It can also **skip** that night's reminders and alarm (`skippedReminderNights`). Tests: `SleepRunwayTests` (a saved night sets that night only; set twice replaces; clearing restores; standing weekday plans survive), `SkippedNightTests`. | 
