...
| Z20 | open | Not started: the calibration harness (held‑out nights vs morning ratings and alertness, error/coverage by strata). No coefficient was changed. | — | Needs a data export from real use to be meaningful. |
| Z21 | partial | The screenshot pipeline (`screenshots.yml`) regenerates iPhone and Watch captures from the exact build and commits them to `docs/screenshots/`. A run on the new head was dispatched for this pass. Not done: increased contrast, VoiceOver audit, 40 mm large text on Watch (`simctl ui content_size` is unsupported on watchOS), the asset/licence ledger update. | Screenshot run results. | VoiceOver pass on device. |
...
| Z23 | P2 | The Today sleep‑debt arc shows `night.sleepDebtMinutes`, the debt **carried into** last night; it doesn't include last night's own shortfall until the next night is written. Planning is fixed (Z01), but the displayed number has the same lag. | Open: changing a displayed headline number touches the Watch, widgets and the week‑ago delta, so it needs its own pass. |
