...
final class VoiceJournalRecorder {
    ...
    func stop() { if audioEngine.isRunning { audioEngine.stop() }; audioEngine.inputNode.removeTap(onBus: 0); request?.endAudio(); task?.cancel(); request = nil; task = nil; isRecording = false }
