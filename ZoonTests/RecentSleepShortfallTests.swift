...
final class RecentSleepShortfallTests: XCTestCase {

    func testNoNightsReturnsNil() {
        XCTAssertNil(RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: [], goalMinutes: 480))
    }

    func testSingleShortNightContributesItsShortfall() {
        guard let debt = RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: [420], goalMinutes: 480) else {
    ...
    func testSurplusNightDoesNotDirectlyCancelExistingDebt() {
        ...
        let withOnlyShort = RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: [420], goalMinutes: 480)!
        let withSurplusAfter = RecentSleepShortfall.debt(
        ...
        XCTAssertEqual(withSurplusAfter, withOnlyShort * RecentSleepShortfall.decayPerNight, accuracy: 0.01)
    }
    ...
    func testOldShortfallFadesGraduallyRatherThanCliffDropping() {
        ...
        let debt = RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: history, goalMinutes: 480)!
    ...
    func testConsistentShortfallConvergesRatherThanGrowingForever() {
        ...
        let expectedSteadyState = 60 / (1 - RecentSleepShortfall.decayPerNight)
    ...
    func testMissingNightsAreSimplyAbsentNotZeroSleep() {
        ...
        guard let debt = RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: [480, 480], goalMinutes: 480) else {
            return XCTFail("expected a debt value")
        }
    ...
    func testSeriesLastElementMatchesScalarDebt() {
        ...
        let series = RecentSleepShortfall.debtSeries(timeAsleepMinutesOldestFirst: oldestFirst, goalMinutes: 480)
        let scalar = RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: oldestFirst.reversed(), goalMinutes: 480)
    ...
    func testEachSeriesElementMatchesDebtOfThatPrefix() throws {
        ...
        for prefixLength in 1...oldestFirst.count {
            ...
            let expected = try XCTUnwrap(RecentSleepShortfall.debt(
    ...
    func testEmptySeriesForEmptyInput() {
        XCTAssertEqual(
            RecentSleepShortfall.debtSeries(timeAsleepMinutesOldestFirst: [], goalMinutes: 480),
            []
        )
    }
    ...
    func testUniformPerNightGoalsMatchScalarGoal() {
        let oldestFirst: [Double] = [480, 420, 390, 500, 460, 300, 480]
        let uniform = RecentSleepShortfall.debtSeries(timeAsleepMinutesOldestFirst: oldestFirst, goalMinutes: 480)
        let perNight = RecentSleepShortfall.debtSeries(
    ...
    func testEachNightIsJudgedAgainstItsOwnGoal() {
        ...
        XCTAssertEqual(series[1], 20 * RecentSleepShortfall.decayPerNight + 80, accuracy: 0.01)
    }
    ...
    func testDebtWithPerNightGoalsMatchesSeriesLastElement() {
        ...
        let scalar = RecentSleepShortfall.debt(
    ...
    func testDebtWithMismatchedCountsReturnsNil() {
        XCTAssertNil(RecentSleepShortfall.debt(timeAsleepMinutesNewestFirst: [480, 420], goalMinutesNewestFirst: [480]))
    }
    ...
    func testGapNightsDecayByCalendarNightsNotByRecords() throws {
        ...
        XCTAssertEqual(debt, 300 * pow(RecentSleepShortfall.decayPerNight, 31), accuracy: 0.01)
    }
    ...
    func testConsecutiveDatesMatchTheUndatedSeries() throws {
        ...
        let undated = RecentSleepShortfall.debtSeries(
            timeAsleepMinutesOldestFirst: oldestFirst,
            goalMinutesOldestFirst: goals
        )
    ...
    func testDatedScalarDebtMatchesDatedSeriesLastElement() throws {
        ...
        XCTAssertEqual(scalar, series.last)
        XCTAssertNil(RecentSleepShortfall.debt(
