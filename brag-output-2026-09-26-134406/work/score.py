# /brag intelligence cut: no instruments. Night wind and crickets, a whoosh on
# each cut, a rising airy swell under the AI scene, soft key taps under the typed
# Coach answer, birdsong at dawn. One room reverb.
import os, wave
import numpy as np
SR = 48000; DUR = 38.5; N = int(SR * DUR); t = np.arange(N) / SR
...
night = 1 - np.clip((t - 35.4) / 2.0, 0, 1)
...
for f, pan, rate, off in ((4300, 0.25, 0.62, 0.1), (4750, 0.75, 0.71, 0.4), (5100, 0.5, 0.83, 0.6)):
    ...
    L += cr * (1 - pan); R += cr * pan
for k, when in enumerate([3.45, 6.85, 11.35, 15.15, 20.75, 26.15, 30.05, 32.95, 35.55]):
...
for i in range(14):   # key taps while the Coach answer types (17.1-17.9 s)
    when = 17.1 + i * 0.058 + rng.random() * 0.015
...
# The AI scene: a band of air that rises as the network lights (7.9-10.6 s).
n = int(3.4 * SR); tt = np.arange(n) / SR; x = rng.standard_normal(n)
lo, mid, hi = band(x, 200, 900), band(x, 900, 2500), band(x, 2500, 7000)
u = np.clip(tt / 3.4, 0, 1)
swell = (lo * (1 - u) + mid * np.sin(np.pi * u) + hi * u) / 3 * np.sin(np.pi * u) ** 1.5 * 0.13
add(swell, 7.9, 0.35); add(np.roll(swell, 700), 7.9, 0.65)
