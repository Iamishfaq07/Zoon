...
night = 1 - np.clip((t - 55.6) / 3.0, 0, 1)          # fades as dawn comes
...
cuts = [3.3, 7.1, 12.0, 16.1, 20.8, 26.3, 31.5, 35.6, 39.7, 43.8, 48.1, 52.0, 55.5, 58.9]
...
when = 55.9
while when < 61.6:
    add(call() * (0.035 + 0.035 * rng.random()) * min(1, (when - 55.7) / 2), when, 0.15 + 0.7 * rng.random())
    when += 0.25 + rng.random() * (1.1 - 0.7 * min(1, (when - 55.7) / 4))
...
# Soft key taps while the Coach answer types (22.8-23.6 s).
for i in range(14):
    n = int(0.025 * SR); tt = np.arange(n) / SR
    add(band(rng.standard_normal(n), 1800, 7000) * np.exp(-tt * 260) * 0.05, 22.8 + i * 0.058 + rng.random() * 0.015, 0.4 + 0.2 * rng.random())
...
with wave.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'score60.wav'), 'wb') as w:
