# The 60-second launch film's sound: a nature soundscape with no musical instruments and
...
night = 1 - np.clip((t - 59.2) / 3.0, 0, 1)          # fades as dawn comes
...
cuts = [3.3, 7.1, 12.0, 16.1, 24.4, 29.9, 35.1, 39.2, 43.3, 47.4, 51.7, 55.6, 59.1, 62.5]
...
when = 59.5
while when < 65.2:
    add(call() * (0.035 + 0.035 * rng.random()) * min(1, (when - 59.3) / 2), when, 0.15 + 0.7 * rng.random())
    when += 0.25 + rng.random() * (1.1 - 0.7 * min(1, (when - 59.3) / 4))

# The AI scene: a band of air that rises as the network lights (8.2-11.6 s).
n = int(3.4 * SR); tt = np.arange(n) / SR; x = rng.standard_normal(n)
lo_, mid_, hi_ = band(x, 200, 900), band(x, 900, 2500), band(x, 2500, 7000)
u2 = np.clip(tt / 3.4, 0, 1)
swell = (lo_ * (1 - u2) + mid_ * np.sin(np.pi * u2) + hi_ * u2) / 3 * np.sin(np.pi * u2) ** 1.5 * 0.13
add(swell, 8.2, 0.35); add(np.roll(swell, 700), 8.2, 0.65)
# Soft key taps while the Coach answer types (26.4-27.2 s).
for i in range(14):
    n = int(0.025 * SR); tt = np.arange(n) / SR
    add(band(rng.standard_normal(n), 1800, 7000) * np.exp(-tt * 260) * 0.05, 26.4 + i * 0.058 + rng.random() * 0.015, 0.4 + 0.2 * rng.random())

# The tour: a soft tap on each touch, an air swish as each screen slides in,
# and a gentle swell as the phone zooms in. Noise only; no pitch.
for when in (17.0, 19.4, 22.0):
    n = int(0.06 * SR); tt = np.arange(n) / SR
    tap = band(rng.standard_normal(n), 300, 2400) * np.exp(-tt * 90) * 0.16
    add(tap, when, 0.55)
for when in (19.55, 22.15):
    n = int(0.45 * SR); tt = np.arange(n) / SR
    sw = band(rng.standard_normal(n), 1200, 6000) * np.sin(np.pi * tt / 0.45) ** 2 * 0.06
    add(sw, when, 0.7); add(np.roll(sw, 300), when, 0.3)
for when in (16.95, 19.85, 22.45):
    n = int(0.8 * SR); tt = np.arange(n) / SR; x = rng.standard_normal(n)
    u3 = tt / 0.8
    zm = (band(x, 250, 1200) * (1 - u3) + band(x, 1200, 4000) * u3) * np.sin(np.pi * u3) ** 2 * 0.05
    add(zm, when, 0.5)
...
with wave.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'score66.wav'), 'wb') as w:
