...
enum ChartLabelLayout {
    ...
    /// Opposite-side captions whose centres sit inside this band still
    /// read as one block. Peak-below at y=44 and dip-above at y=76 on the
    /// 3:27 PM screenshot had a 4 pt box gap and still wrote through each
    /// other on the falling slope.
    static let defaultOppositeBand: Double = 40
    ...
    static func place(
        ...
        oppositeBand: Double = defaultOppositeBand
    ) -> [Placement] {
        ...
        for index in order {
            ...
            func hits(_ box: Box, side: Side, show: Bool) -> Bool {
                ...
                return placed.contains {
                    $0.placement.showCaption && $0.box.intersects(
                        box,
                        gap: gap,
                        sameSide: $0.placement.side == side,
                        oppositeBand: oppositeBand
                    )
                }
            }
    ...
    private struct Box {
        ...
        func intersects(_ other: Box, gap: Double, sameSide: Bool, oppositeBand: Double) -> Bool {
            let xOverlap = minX < other.maxX + gap && other.minX < maxX + gap
            guard xOverlap else { return false }
            ...
            let yOverlap = minY < other.maxY + gap && other.minY < maxY + gap
            if yOverlap { return true }
            let cy = (minY + maxY) / 2
            let otherCy = (other.minY + other.maxY) / 2
            return abs(cy - otherCy) < oppositeBand
        }
    }
