...
| §12 Swift 6 language mode | Stage 2 done, not flipped | Complete strict-concurrency checking went from 65 diagnostics to **0** and is now an enforced CI gate (Release job). Fixes: immutable intent metadata, `InferSendableFromCaptures` for key paths, `@MainActor` haptics, `Sendable` insight engines and rules, documented `nonisolated(unsafe)` for WatchConnectivity payloads, the HealthKit observer completion and one formatter. One unjustified `@unchecked Sendable` fixed; the other four are lock-guarded. A report-only build in **Swift 6 language mode also compiles with 0 errors and 0 warnings** (app, widget, watch). The switch itself is deliberately left for after device QA: Swift 6 adds runtime isolation checks that can trap when a system callback arrives off the expected actor, and nothing here has run on a device. When it is flipped, change `SWIFT_VERSION` in `Tools/generate-pbxproj.py` and `Tools/check-build-settings.py` together. |
