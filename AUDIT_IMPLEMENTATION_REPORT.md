...
| **Coach** | The Apple Intelligence insight's `likelyCause` was free text: numbers were grounded, causes were not (it could name caffeine nobody logged). "Do I have sleep apnea?" contained "sleep" and was answered with last night's duration. No evaluation corpus. | The model returns a `driverID` chosen from the rules that actually fired tonight; the shown sentence is the rule's own; an unoffered id rejects the whole generation back to rules. Medical-condition questions get a fixed referral that neither diagnoses nor reassures. 22 questions × 5 night types evaluated on every CI run; diagnosis, unknown-evidence, invented-number and instruction-leak rates are required to be 0. |
| **Snore** | Confidence came from detector agreement only: 31 minutes of monitoring an 8-hour night could read "High confidence, no snoring". | Final confidence = min(detector, **coverage**), coverage = monitored ÷ intended window (tonight's sleep opportunity), less interruptions. Low coverage with no events says "No conclusion — only N% of the night was monitored." Coverage % stored with each night. |
...
| **UI / accessibility** | Accessibility audit logged and passed. No String Catalog. No long-string or right-to-left check. | Audit is a ratchet gate: a new kind of issue on a tab fails immediately; known kinds (contrast, Dynamic Type, clipping) may not grow past the per-tab baseline. String Catalogs in app, widget and watch. A UI test launches with doubled-length strings and forced RTL and must still reach every tab. Personal learning shows an "unlock map" built from the engines' own thresholds. |
...
8. **"High confidence, no snoring" from 31 minutes of an 8-hour night.** `d38c152`
...
| `ShortfallSemanticsTests` | before/added/through accessors; step equals series; Coach: short night after a clean run, modest short night, long night inherits, met need, unknown need, historical night uses its own figure, evidence labels; DST nights |
...
| `SnoreCoverageTests` | 31 min of 8 h is not high; 7 clean hours can be; interruptions lower coverage; final = weaker of two; no-snore on low coverage says no conclusion; never a medical finding; ratio bounded |
...
- Gate baseline, per tab (contrast / Dynamic Type / clipped text): Today
  16/6/4, Sleep 22/9/4, Insights 7/11/0, Coach 10/17/0, plus a two-line
  allowance for date-relative demo labels. Any other kind fails outright.
- The remaining Dynamic Type findings come from deliberate
  `.dynamicTypeSize(...)` clamps on rings, tiles and the tab bar, and the
  contrast findings from secondary text on the dark theme. Both are listed
  for fixing, not fixed in this pass (see below).
- New UI in this pass (Data Quality read problems, unlock map) carries state
  in text as well as symbol or colour, and combines each row for VoiceOver.
...
- **Accessibility gate allows two lines of slack** per bucket for
  date-dependent demo labels; a regression of one line in a bucket can pass.
...
| §11 split `SleepDataCoordinator` | Not done | A behaviour-preserving split of a 3,000-line coordinator needs an iteration loop faster than a 20-minute CI round trip and device checks of sync. The reads it owns are now typed, which is the first seam a split would cut along. |
| §12 Swift 6 language mode | Staged, not flipped | Complete strict-concurrency checking runs in CI as a report (Release job); the count is the to-do list. One unjustified `@unchecked Sendable` fixed; the other four are lock-guarded and documented. |
...
| §17.1–17.4, 17.6 UI redesign, §18 Sound Studio, §19 Snore results redesign, §20 readiness, §21 room sound | Not done | Design work that needs looking at the result on a device; the audit orders it after P0/P1. §17.5 (unlock map) is done. |
